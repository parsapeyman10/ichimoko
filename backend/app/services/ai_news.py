...
class AiNewsAnalyzer:
    ...
    async def analyze(self, articles: list[Headline], source_state: str, guard: dict,
        ...
        if not self.settings.openai_api_key or not self.settings.ai_news_external_consent:
            ...
            try:
                ...
                if impact == "HIGH" or direction == "NEUTRAL" or confidence < 80:
                    ...
                    verdict = {"status": "AVAILABLE", "symbol": "XAU/USD", "direction": direction,
                               "confidence": confidence, "impact": impact, "model": self.settings.openai_model,
    ...
    async def _request(self, candidates: list[Headline]) -> dict:
        """One bounded aggregate classification; override this in tests without a real model key."""
        from openai import AsyncOpenAI
        client = AsyncOpenAI(api_key=self.settings.openai_api_key, timeout=7, max_retries=0)
        snippets = [{"id": a.id, "title": a.headline[:240], "excerpt": a.summary[:160],
        ...
        response = await client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": (
                    "You evaluate near-term XAU/USD macro-news context ONLY. Titles/excerpts are "
